...
export default function AdminPlaylistsForm({ playlist }: PageProps) {
  const isEdit = !!playlist

  const [showExistingThumbnail, setShowExistingThumbnail] = useState(!!playlist?.asset?.url)
  ...
  return (
    <>
      <Head title={isEdit ? 'Edit Playlist' : 'Create Playlist'} />
      <Header fixed>
        <Breadcrumb>
          <BreadcrumbList>
            <BreadcrumbItem>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbPage>{isEdit ? 'Edit Playlist' : 'Create Playlist'}</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
      </Header>
      <Main>
        <Form
          route={isEdit ? 'admin.playlists.update' : 'admin.playlists.create'}
          routeParams={{ id: isEdit ? playlist.id : undefined }}
        >
          {({ processing }) => (
            <div className="flex flex-col gap-8 py-4">
              ...
              <Section
                title="Content"
                description="Title, description, and details of the playlist"
              >
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <Field name="name">
                    ...
                    <Input
                      ...
                      minLength={3}
                      maxLength={255}
                      defaultValue={playlist?.name ?? undefined}
                      aria-label="Name"
                    />
                    <FieldError />
                  </Field>
                  <Field name="slug">
                    ...
                    <Input
                      ...
                      id="slug"
                      maxLength={255}
                      defaultValue={playlist?.slug ?? undefined}
                      aria-label="Slug"
                    />
                    <FieldError />
                  </Field>
              ...
              <Section title="Publishing" description="Control visibility and status">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <Field name="state">
                    ...
                    <Select name="state" defaultValue={playlist?.state ?? States.DRAFT}>
                      <SelectTrigger>
                ...
                <Field name="isFeatured">
                  <div className="flex items-center gap-2">
                    <Switch
                      id="isFeatured"
                      name="isFeatured"
                      defaultChecked={playlist?.isFeatured}
                    />
              ...
              {/* Thumbnail */}
              <Section title="Thumbnail" description="Playlist thumbnail image and metadata">
                  ...
                  <Field name="thumbnail.file">
                    <Button
                      ...
                      className={`relative flex cursor-pointer flex-col items-center justify-center overflow-hidden border-2 border-dashed transition-colors ${
                        thumbnailFiles.length > 0 || (showExistingThumbnail && playlist?.asset?.url)
                          ? '!h-auto min-h-0 !p-0'
                          : 'min-h-36'
                      } ${
                        isDragging
                    ...
                    >
                      ...
                      ) : showExistingThumbnail && playlist?.asset?.url ? (
                        <div className="group relative w-full">
                          <img
                            src={playlist.asset.url}
                            alt={playlist.asset.altText || 'Current thumbnail'}
                            className="block max-h-64 w-full rounded-[5px] object-cover"
                          />
                          <div className="absolute inset-0 flex items-center justify-center rounded-[5px] bg-black/0 transition-colors group-hover:bg-black/30">
                            <span className="rounded-md bg-background/90 px-3 py-1.5 text-xs font-medium text-foreground opacity-0 shadow-sm transition-opacity group-hover:opacity-100">
                ...
                <Field name="thumbnail.altText">
                  ...
                  <Input
                    ...
                    defaultValue={playlist?.asset?.altText ?? undefined}
                  />
                  <FieldError />
                </Field>
                <Field name="thumbnail.credit">
                  ...
                  <Input
                    ...
                    defaultValue={playlist?.asset?.credit ?? undefined}
                  />
                  <FieldError />
                </Field>
              </Section>
              ...
              <Section title="SEO" description="Search engine optimization metadata">
                <Field name="pageTitle">
                  ...
                  <Input
                    ...
                    id="pageTitle"
                    maxLength={255}
                    defaultValue={playlist?.pageTitle ?? undefined}
                ...
                <Field name="metaDescription">
                  ...
                  <Textarea
                    ...
                    rows={3}
                    maxLength={255}
                    defaultValue={playlist?.metaDescription ?? undefined}
...
AdminPlaylistsForm.layout = (page: React.ReactElement) => <AdminLayout>{page}</AdminLayout>
