...
export default function AdminSeriesForm({ series, taxonomies }: PageProps) {
  const isEdit = !!series

  const [showExistingThumbnail, setShowExistingThumbnail] = useState(!!series?.asset?.url)
  ...
  const defaultTaxonomyIds = Array.isArray(series?.taxonomies)
    ? series.taxonomies.map((t: any) => t.id)
    : []
  ...
  return (
    <>
      ...
      <Main>
        <Form
          route={isEdit ? 'admin.series.update' : 'admin.series.create'}
          routeParams={{ id: isEdit ? series.id : undefined }}
        >
          {({ processing }) => (
            <div className="flex flex-col gap-8 py-4">
              {/* Content */}
              <Section title="Content" description="Title, description, and details of the series">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <Field name="name">
                    ...
                    <Input
                      ...
                      minLength={3}
                      maxLength={255}
                      defaultValue={series?.name ?? undefined}
                      aria-label="Name"
                    />
                    <FieldError />
                  </Field>
                  <Field name="slug">
                    ...
                    <Input
                      ...
                      id="slug"
                      maxLength={255}
                      defaultValue={series?.slug ?? undefined}
                      aria-label="Slug"
                    />
                    <FieldError />
                  </Field>
                ...
                <Field name="description">
                  ...
                  <Textarea
                    ...
                    rows={3}
                    maxLength={255}
                    defaultValue={series?.description ?? undefined}
              ...
              <Section title="Publishing" description="Control visibility and status">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <Field name="state">
                    ...
                    <Select name="state" defaultValue={series?.state ?? States.DRAFT}>
                      <SelectTrigger>
                ...
                <Field name="isFeatured">
                  <div className="flex items-center gap-2">
                    <Switch id="isFeatured" name="isFeatured" defaultChecked={series?.isFeatured} />
              ...
              {/* Taxonomies */}
              <Section title="Taxonomies" description="Assign taxonomies to this series">
              ...
              <Section title="Thumbnail" description="Course thumbnail image and metadata">
                  ...
                  <Field name="thumbnail.file">
                    <Button
                      ...
                      className={`relative flex cursor-pointer flex-col items-center justify-center overflow-hidden border-2 border-dashed transition-colors ${
                        thumbnailFiles.length > 0 || (showExistingThumbnail && series?.asset?.url)
                          ? '!h-auto min-h-0 !p-0'
                          : 'min-h-36'
                      } ${
                        isDragging
                    ...
                    >
                      ...
                      ) : showExistingThumbnail && series?.asset?.url ? (
                        <div className="group relative w-full">
                          <img
                            src={series.asset.url}
                            alt={series.asset.altText || 'Current thumbnail'}
                            className="block max-h-64 w-full rounded-[5px] object-cover"
                          />
                          <div className="absolute inset-0 flex items-center justify-center rounded-[5px] bg-black/0 transition-colors group-hover:bg-black/30">
                            <span className="rounded-md bg-background/90 px-3 py-1.5 text-xs font-medium text-foreground opacity-0 shadow-sm transition-opacity group-hover:opacity-100">
                ...
                <Field name="thumbnail.altText">
                  ...
                  <Input
                    ...
                    defaultValue={series?.asset?.altText ?? undefined}
                  />
                  <FieldError />
                </Field>
                <Field name="thumbnail.credit">
                  ...
                  <Input
                    ...
                    defaultValue={series?.asset?.credit ?? undefined}
                  />
                  <FieldError />
                </Field>
              </Section>
              ...
              <Section title="SEO" description="Search engine optimization metadata">
                <Field name="pageTitle">
                  ...
                  <Input
                    ...
                    id="pageTitle"
                    maxLength={255}
                    defaultValue={series?.pageTitle ?? undefined}
                ...
                <Field name="metaDescription">
                  ...
                  <Textarea
                    ...
                    rows={3}
                    maxLength={255}
                    defaultValue={series?.metaDescription ?? undefined}
...
AdminSeriesForm.layout = (page: React.ReactElement) => <AdminLayout>{page}</AdminLayout>
