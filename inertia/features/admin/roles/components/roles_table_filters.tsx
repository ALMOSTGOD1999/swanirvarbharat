...
export default function RolesTableFilters({
  querySearch,
  setQuerySearch,
  dateFrom,
  setDateFrom,
  ...
  const buildParams = React.useCallback(
    ...
    [querySearch, dateFrom, dateTo, limit]
  )

  const handleSubmit = React.useCallback((params: Record<string, any>) => {
    router.get(urlFor('admin.roles.index'), params, {
      preserveScroll: true,
      preserveState: true,
      replace: true,
  ...
  const isFiltered = querySearch.trim().length > 0 || !!dateFrom || !!dateTo
