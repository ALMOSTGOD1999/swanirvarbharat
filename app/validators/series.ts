...
export const createSeriesValidator = vine.create({
  name: name(),
  slug: slug()
...
export const updateSeriesValidator = vine.create({
  name: name().optional(),
  slug: slug()
    .unique(async (db, value, field) => {
      const result = await db
...
export const seriesPostValidator = vine.create({
  postIds: vine
    .array(vine.string().fixedLength(24).exists({ table: 'posts', column: 'id' }))
    .optional(),
})

export const reorderSeriesPostsValidator = vine.create({
