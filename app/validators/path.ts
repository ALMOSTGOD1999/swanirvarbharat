...
export const pathIndexValidator = vine.create({
...
export const createPathValidator = vine.create({
  name: name(),
  slug: slug()
...
export const updatePathValidator = vine.create({
  name: name().optional(),
  slug: slug()
    .unique(async (db, value, field) => {
      const result = await db
...
export const pathCourseValidator = vine.create({
  courseIds: vine
    .array(vine.string().fixedLength(24).exists({ table: 'courses', column: 'id' }))
    .optional(),
})

export const reorderPathCoursesValidator = vine.create({
  courseIds: vine.array(vine.string().fixedLength(24)),
})
